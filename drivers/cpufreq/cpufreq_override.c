...
void	cpufreq_screenstate_getdefvoltage(u8 dvolt[]),
	cpufreq_screenstate_setdefvoltage(u8 dvolt[]);
...
} ovrdcfg = { 0, 0, 0, 0, 0, 0, HIGHTEMP_SCALEBACK, LOWTEMP_RESET,
		BATTERY_PERCENT,SCALEBACK_SPEED};
...
static ssize_t show_hightemp_scaleback(struct cpufreq_policy *policy, char *buf)
{
...
CPUFREQ_OVERRIDE_ATTR(battery_scaleback_percent,
		      show_battery_scaleback_per, store_battery_scaleback_per);
